...
class Shape():
    def __init__(self, center=geometry.point(), obstacle_id=None):
        if center is None: raise ValueError()
        self.center = center
        self.rotmat = geometry.identity()
        self.obstacle_id = obstacle_id  # only store the string, so shape is pickle-able
...
class Circle(Shape):
    def __init__(self, center=geometry.point(), radius=25/2, obstacle_id=None):
        super().__init__(center, obstacle_id=obstacle_id)
        self.radius = radius
        self.orient = 0.
...
class Rectangle(Polygon):
    def __init__(self, center=None, dimensions=None, orient=0, obstacle_id=None):
        ...
        super().__init__(vertices=vertices, orient=orient, obstacle_id=obstacle_id)
